...
static int add_chunk(xjni_pool_internal* ip) {
	...
	for (size_t i = 0; i < cap; ++i) {
		void* elem =
			(uint8_t*)memory + i * ip->base.element_size;
...
xjni_pool* xjni_pool_Create(size_t capacity,
							size_t element_size,
							int use_mutex) {
	...
	xjni_pool_internal* ip =
		(xjni_pool_internal*)calloc(1, sizeof(*ip));
	if (!ip)
		return NULL;
	...
	if (!add_chunk(ip)) {
		if (use_mutex)
			xjni_mutex_destroy(&ip->base.mutex);
		free(ip);
		return NULL;
	}
...
size_t xjni_pool_UsedCount(xjni_pool* mp) {
	...
	if (ip->lock_free)
		return atomic_load_explicit(&ip->atomic_size,memory_order_relaxed);
