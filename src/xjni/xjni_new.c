...
JNIEXPORTC jfloatArray JNICALL xjni_NewFloatArray(JNIEnv* env,const jfloat* data,jsize len) {
	if (!env || len < 0) return NULL;
...
JNIEXPORTC jdoubleArray JNICALL xjni_NewDoubleArray(JNIEnv* env,const jdouble* data,jsize len) {
	if (!env || len < 0) return NULL;
