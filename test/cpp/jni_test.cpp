...
static bool clearException(JNIEnv *e, std::string *out = nullptr) {
	...
	if (out && x) {
		...
		jmethodID m = c ? e->GetMethodID(c, "toString", "()Ljava/lang/String;") : nullptr;
		if (m) {
...
static void testJavaVM(JNIEnv *env, TestState &t) {
	JavaVM *raw = nullptr;
	t.check(env->GetJavaVM(&raw) == JNI_OK && raw != nullptr,"JNIEnv::GetJavaVM");
	jni::XJavaVM vm(raw);
	t.check(static_cast<bool>(vm) && vm.get() == raw, "XJavaVM basics");
	JNIEnv *current = nullptr;
	t.check(vm.GetEnv(&current, JNI_VERSION_1_8) == JNI_OK && current == env,"XJavaVM::GetEnv");
...
static void testConstructionAndMethods(JNIEnv *env, TestState &t) {
	...
	if (ab) x.DeleteLocalRef(ab);
	if (a) x.DeleteLocalRef(a);
	if (b) x.DeleteLocalRef(b);
	if (so) x.DeleteLocalRef(so);
	if (o) x.DeleteLocalRef(o);
	if (c) x.DeleteLocalRef(c);
}
...
static void testAllPrimitiveMethods(JNIEnv *env, TestState &t) {
	...
	t.check(x.CallLongMethod(o, x.GetMethodID(c, "addLong", "(JJ)J"), 2LL, 3LL) == 5,"CallLongMethod");
	t.check(std::fabs(x.CallFloatMethod(o, x.GetMethodID(c, "addFloat", "(FF)F"),1.25f, 2.5f) - 3.75f) < 1e-6,"CallFloatMethod");
	t.check(std::fabs(x.CallDoubleMethod(o, x.GetMethodID(c, "addDouble", "(DD)D"), 1.25, 2.5) - 3.75) < 1e-12,"CallDoubleMethod");
	t.check(x.CallByteMethod(o, x.GetMethodID(c, "addByte", "(BB)B"), (jbyte)2,(jbyte)3) == 5,"CallByteMethod");
	t.check(x.CallShortMethod(o, x.GetMethodID(c, "addShort", "(SS)S"), (jshort)2,(jshort)3) == 5,"CallShortMethod");
	t.check(x.CallCharMethod(o, x.GetMethodID(c, "nextChar", "(C)C"),(jchar)'A') == 'B',"CallCharMethod");
	t.check(x.CallBooleanMethod(o, x.GetMethodID(c, "and", "(ZZ)Z"), JNI_TRUE,JNI_FALSE) == JNI_FALSE,"CallBooleanMethod");
	if (o) x.DeleteLocalRef(o);
	if (c) x.DeleteLocalRef(c);
}
...
static void testStatic(JNIEnv *env, TestState &t) {
	...
	t.check(std::fabs(
		x.CallStaticDoubleMethod(c, x.GetStaticMethodID(c, "staticDouble", "(DD)D"), 1., 2.) - 3.) < 1e-12,"CallStaticDoubleMethod");
	t.check(x.CallStaticByteMethod(c,x.GetStaticMethodID(c, "staticByte", "(BB)B"),(jbyte)2, (jbyte)3) == 5,"CallStaticByteMethod");
	t.check(
		x.CallStaticShortMethod(c, x.GetStaticMethodID(c, "staticShort", "(SS)S"),(jshort)2, (jshort)3) == 5,"CallStaticShortMethod");
	t.check(x.CallStaticCharMethod(c,x.GetStaticMethodID(c, "staticChar", "(C)C"),(jchar)'A') == 'C',"CallStaticCharMethod");
	t.check(
		x.CallStaticBooleanMethod(c, x.GetStaticMethodID(c, "staticAnd", "(ZZ)Z"),
			JNI_TRUE, JNI_TRUE) == JNI_TRUE,"CallStaticBooleanMethod");
	jstring a = x.NewStringUTF("X"), b = x.NewStringUTF("Y");
	jstring r = static_cast<jstring>(x.CallStaticObjectMethod(c,x.GetStaticMethodID(
		c, "staticConcat","(Ljava/lang/String;Ljava/lang/String;)Ljava/lang/String;"),a, b));
	t.check(x.GetStringUTF(r) == "XY", "CallStaticObjectMethod");
	jobject o = x.NewObject(c, x.GetMethodID(c, "<init>", "()V"));
	x.CallStaticVoidMethod(c, x.GetStaticMethodID(c, "staticSetInt", "(LJNITestObject;I)V"), o, 321);
	t.check(x.GetIntField(o, x.GetFieldID(c, "intField", "I")) == 321,"CallStaticVoidMethod");
	t.check(x.CallStaticMethod<jint>(c, x.GetStaticMethodID(c, "staticAdd", "(II)I"), 7, 8) == 15,"CallStaticMethod<T>");
	if (r) x.DeleteLocalRef(r);
	if (a) x.DeleteLocalRef(a);
	if (b) x.DeleteLocalRef(b);
	if (o) x.DeleteLocalRef(o);
	if (c) x.DeleteLocalRef(c);
...
static void testNonvirtual(JNIEnv *env, TestState &t) {
	...
	t.check(x.CallNonvirtualIntMethod(o, base, vm) == 10,"CallNonvirtualIntMethod");
	...
	t.check(x.CallNonvirtualMethod<jint>(o, base, vm) == 10,"CallNonvirtualMethod<T>");
	if (s) x.DeleteLocalRef(s);
	if (o) x.DeleteLocalRef(o);
	if (base) x.DeleteLocalRef(base);
	if (der) x.DeleteLocalRef(der);
}
...
#define SFIELD(name, sig, GET, SET, val) \
	...
	SFIELD("staticBooleanField", "Z", GetStaticBooleanField,SetStaticBooleanField, JNI_FALSE);
	SFIELD("staticByteField", "B", GetStaticByteField, SetStaticByteField,(jbyte)8);
	SFIELD("staticCharField", "C", GetStaticCharField, SetStaticCharField,(jchar)'Q');
	SFIELD("staticShortField", "S", GetStaticShortField, SetStaticShortField,(jshort)18);
	SFIELD("staticIntField", "I", GetStaticIntField, SetStaticIntField, (jint)28);
	SFIELD("staticLongField", "J", GetStaticLongField, SetStaticLongField,(jlong)38);
	SFIELD("staticFloatField", "F", GetStaticFloatField, SetStaticFloatField,(jfloat)48.5f);
	SFIELD("staticDoubleField", "D", GetStaticDoubleField, SetStaticDoubleField,(jdouble)58.5);
#undef SFIELD
	jfieldID sof = x.GetStaticFieldID(c, "staticObjectField", "Ljava/lang/String;");
	...
	if (sg) x.DeleteLocalRef(sg);
	if (ss) x.DeleteLocalRef(ss);
	if (o) x.DeleteLocalRef(o);
	if (c) x.DeleteLocalRef(c);
}
...
static void testArrays(JNIEnv *env, TestState &t) {
	...
	t.check(x.GetStringUTF((jstring)e0) == "one" && x.GetStringUTF((jstring)e1) == "two","object array");
	if (e0) x.DeleteLocalRef(e0);
	if (e1) x.DeleteLocalRef(e1);
	if (s1) x.DeleteLocalRef(s1);
	if (s2) x.DeleteLocalRef(s2);
	if (strc) x.DeleteLocalRef(strc);
	if (ba) x.DeleteLocalRef(ba);
	if (bya) x.DeleteLocalRef(bya);
	if (ca) x.DeleteLocalRef(ca);
	if (sa) x.DeleteLocalRef(sa);
	if (ia) x.DeleteLocalRef(ia);
	if (la) x.DeleteLocalRef(la);
	if (fa) x.DeleteLocalRef(fa);
	if (da) x.DeleteLocalRef(da);
	if (oa) x.DeleteLocalRef(oa);
}
...
static void testContainers(JNIEnv *env, TestState &t) {
	...
	li.push_back("a");
	li.push_back("b");
	jobject jl = x.CppListToJavaList(env, li);
...
static void testStreams(JNIEnv *env, TestState &t) {
	...
	jni::JavaIStream cin = x.GetIStream(bis);
	char b[6] = {};
	cin->read(b, 5);
	...
	jni::JavaOStream cout = x.GetOStream(bos);
	(*cout) << "hello" << " world";
	cout->flush();
...
static void testFStreams(JNIEnv* env, TestState& t) {
	jni::XJNIEnv x(env);
	jclass bic = env->FindClass("java/io/ByteArrayInputStream");
	jmethodID bicc = x.GetMethodID(bic, "<init>", "([B)V");
	jbyteArray data = x.NewByteArray(11);
	...
	x.SetByteArrayRegion(data, 0, 11, text);
	jobject bis = x.NewObject(bic, bicc, data);
	jni::JavaFileIStream cin = x.GetIFStream(bis);
	char b[6] = {};
	cin->read(b, 5);
	t.check(std::string(b, 5) == "hello", "JavaFileIStream", std::string(b, 5));
	if (bis) x.DeleteLocalRef(bis);
	if (data) x.DeleteLocalRef(data);
	if (bic) x.DeleteLocalRef(bic);
	jclass boc = env->FindClass("java/io/ByteArrayOutputStream");
	jmethodID bocc = x.GetMethodID(boc, "<init>", "()V");
	jobject bos = x.NewObject(boc, bocc);
	jni::JavaFileOStream cout = x.GetOFStream(bos);
	(*cout) << "hello" << " world";
	cout->flush();
	jmethodID to = x.GetMethodID(boc, "toString", "()Ljava/lang/String;");
	jstring got = static_cast<jstring>(x.CallObjectMethod(bos, to));
	t.check(x.GetStringUTF(got) == "hello world", "JavaFileOStream", x.GetStringUTF(got));
	if (got) x.DeleteLocalRef(got);
	if (bos) x.DeleteLocalRef(bos);
	if (boc) x.DeleteLocalRef(boc);
	jobject ji = x.NewIFStream(xfmkstemp("hello world"));
	jobject jo = x.NewOFStream(xomkstemp());
	t.check(ji && jo, "NewIFStream/NewOFStream");
	if (ji) x.DeleteLocalRef(ji);
	if (jo) x.DeleteLocalRef(jo);
}
